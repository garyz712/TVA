...
#             elif prec == 1:  # INT8
#                 a_val = float(np.int16(raw_a & 0xFF00) >> 8) / (2**7)
#             else:  # FP16
...
#                 elif prec == 1:
#                     v_val = float(np.int16(raw_v & 0xFF00) >> 8) / (2**7)
#                 else:
...
async def run_test_case(dut, a_mem, v_mem, precision_sel, test_name):
    dut._log.info(f"\n{test_name}")
    # Drive inputs
    for k in range(NUM_COLS):
...
async def test_attention_av_multiply(dut):
    ...
    precision_sel = [2] * NUM_COLS  # FP16 for maximum precision
    for k in range(NUM_COLS):
        for i in range(A_ROWS):
            a_mem[i, k] = real_to_q1_15(1.0 if (i + k) % 2 == 0 else -1.0)  # Max/min values
        for j in range(V_COLS):
    ...
    # Test Case 6: Random Precision
    precision_sel = [random.randint(0, 2) for _ in range(NUM_COLS)]  # Random precisions
    for k in range(NUM_COLS):
        for i in range(A_ROWS):
            a_mem[i, k] = real_to_q1_15(random.uniform(-0.9, 0.9))
        for j in range(V_COLS):
